...
#define TOM_FREQ            10000.0f                    /* TOM frequency   --> 0.1ms                                 */
#define SERVO_DOOR          &MODULE_P00, 11          /* LED which will be toggled in Interrupt Service Routine (ISR) */

/*********************************************************************************************************************/
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
IfxGtm_Tom_Timer g_timerDriver;                     /* TOM driver                                                   */
uint32 g_on_time = 21;      // ~= 2ms
/*********************************************************************************************************************/
...
void interruptGtmTom(void)
{
    ...
    if (counter == 0) {
        IfxPort_setPinHigh(SERVO_DOOR);
    }
    else if (counter == g_on_time) {
        IfxPort_setPinLow(SERVO_DOOR);
    }
...
//    IfxGtm_Cmu_enableClocks(&MODULE_GTM, IFXGTM_CMU_CLKEN_FXCLK);               /* Enable the CMU clock             */
    ...
    IfxPort_setPinModeOutput(SERVO_DOOR, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);  /* Set pin mode         */
